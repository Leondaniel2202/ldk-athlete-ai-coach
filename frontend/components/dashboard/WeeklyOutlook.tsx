        ...
        {workouts.map((workout) => (
          ...
          >
            <p className="text-sm font-semibold text-zinc-700">
              {workout.date_start
                ? new Date(workout.date_start).toLocaleDateString(undefined, {
                    weekday: "short",
                    month: "short",
                    day: "numeric",
                  })
            ...
            <div>
              <h4 className="text-sm font-semibold text-zinc-950">{workout.name}</h4>
              <p className="mt-1 text-sm text-zinc-600">{workout.category ?? "—"}</p>
            </div>
